...
import { getUserData, sendImagen } from "../../api"; // Asegúrate de importar correctamente la función
...
const Profile = () => {
  ...
  const [profileImage, setProfileImage] = useState(
    "https://image.cdn2.seaart.ai/2023-06-29/39615905747013/4c7b80cbc5afe7b10d0a6d39ec94b73798b652d7_high.webp"
  );
  const [name, setName] = useState("");
  const [lastName, setLastName] = useState("");
  const [email, setEmail] = useState("");
  const [photo, setPhoto] = useState<any>(null)
  const [cdlPhoto, setCdlPhoto] = useState<any>(null)
  ...
  const handleImagePick = async (option) => {
    ...
    if (status === "granted") {
      ...
      if (!result.canceled) {
        ...
        try{
          const token = await AsyncStorage.getItem("authToken");
          const res = await sendImagen(token, base64String) 
          
          res.success?  [console.log(res), await fetchUserData()]:Alert.alert("Ah ocurrido un error");

          console.log(res)

        }
        catch(error){
          Alert.alert(' Error', 'Hubo un problema al enviar la imagen. Inténtalo de nuevo.');
          console.log(error)
        }

      }
    } else {
      Alert.alert("Permisos denegado");
    }
  };
    ...
    <View style={styles.container}>
      ...
      <View style={styles.header}>
        <View style={styles.profileImageContainer}>
          {
            photo? <AdvancedImage cldImg={ cld.image(photo) } className="w-[100px] h-[100px] rounded-full " />
